...
OUT_ROUTER_PARTIAL = REPO_ROOT / "AspireAPI" / "Generated" / "GeneratedToolRouterRegistration.cs"
...
def render_router_partial(tool_names: list[str]) -> str:
    """Static extension method that registers every generated tool on a router.
    Both AspireMcpServer (stdio mode) and the admin web layer use the same router
    by going through this extension."""
    body = "\n".join(
        f"            router.RegisterTool(\"{n}\", sp => sp.GetRequiredService<AspireAPI.Generated.Handlers.{n}Handler>());"
        for n in tool_names
    )
    return f"""// <auto-generated/>
using Microsoft.Extensions.DependencyInjection;
...
{body}
        ...
        public static System.Collections.Generic.IReadOnlyDictionary<string, System.Type> KnownGeneratedTools {{ get; }} =
            ...
            {{
{",\n".join(f'                {{ "{n}", typeof(AspireAPI.Generated.Handlers.{n}Handler) }}' for n in tool_names)}
            }};
    }}
}}
"""
